    ...
    */
    ...
    var Music = {
        ...
        songs: [
                ...
                {
                    name: 'Pretty Girl',
                    singer_first: 'JanieMaggie Lindemannck',
                    singer_second: '',
                    singer_third: '',
        ...
        const _this = this;
            ...
            // Xử lý khi click vào nút "Phát Tất Cả"
            const playAllButton = document.getElementById('playAllButton');
            playAllButton.addEventListener('click', function () {
                // Bắt đầu phát tất cả các bài hát từ đầu
                _this.currenIndex = 0;
            ...
            // Xử lý khi click vào nút phát "PlayList và Album"
            function handlePlayButtonClick(songIndex, buttonId) {
                const playButton = document.getElementById(buttonId);
                const playIcon = playButton.querySelector('.controls_player--icon_img.play');

                playButton.addEventListener('click', function () {
                    // Bắt đầu phát bài hát
                    _this.currenIndex = songIndex;
                    _this.loadCrurentSong();
                    audio.play();
                    _this.render();

                    // // Dừng sau khi phát xong bài này
                    // audio.onended = function () {
                    //     audio.pause();
                    //     _this.isplaying = false;
                    //     _this.render();
                    //     playIcon.querySelector('i').className = 'far fa-play-circle'; // Đổi icon khi hết nhạc
                    // };

                    // Nếu đang phát, thì dừng lại, chuyển sang icon play
                    if (_this.isplaying) {
                        audio.pause();
                        playIcon.querySelector('i').className = 'far fa-play-circle';
                    } else {
                        // Nếu đang dừng, thì bắt đầu phát, chuyển sang icon pause
                        audio.play();
                        playIcon.querySelector('i').className = 'far fa-pause-circle';
                    }

                    _this.isplaying = !_this.isplaying; // Chuyển đổi trạng thái isplaying
                    _this.render();
                });
            }

            // Sử dụng hàm xử lý chung cho nút phát "Hoa Cỏ Lau"
            handlePlayButtonClick(3, 'playHoaCoLau');

            //Xử lý phát phần Album
            handlePlayButtonClick(17, 'playAlbum1');
            handlePlayButtonClick(18, 'playAlbum2');
            handlePlayButtonClick(19, 'playAlbum3');
            handlePlayButtonClick(20, 'playAlbum4');
            handlePlayButtonClick(21, 'playAlbum5');
            handlePlayButtonClick(22, 'playAlbum6');
            handlePlayButtonClick(23, 'playAlbum7');
            handlePlayButtonClick(24, 'playAlbum8');
       ...
       nextSong: function() {
           if (this.currenIndex === 16) {
               this.currenIndex = 0; // Nếu đến cuối danh sách thì quay lại bài đầu tiên
           } else if (this.currenIndex < 16){
               this.currenIndex++;
           } else{
       ...
       prevSong: function(){
            ...
            if(this.currenIndex < 0){
                this.currenIndex = this.songs.length - 9;
            } 
            else if(this.currenIndex == 16){
       ...
       playRandomSong: function(){
           let newIndex;
            do{
                newIndex = Math.floor(Math.random() * (this.songs.length-8));
